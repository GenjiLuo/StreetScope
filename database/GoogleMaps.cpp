...
bool ImageDownloader::getPanoNear (Location const& location, Panorama& panorama) {
   ...
   _strings.clear();
   panorama.edges.removeAll();

   // fetch xml data from google maps
   cout << "Finding pano near " << location << "...";
   ostringstream url;
...
bool ImageDownloader::read_sv_xml (Panorama& panorama) {
   ...
   // extract plain old data
   panorama.location.lat() = data_node.attribute("lat").as_float();
   panorama.location.lon() = data_node.attribute("lng").as_float();
   panorama.origLocation.lat() = data_node.attribute("original_lat").as_double();
   panorama.origLocation.lon() = data_node.attribute("original_lng").as_double();
   panorama.panoYaw = proj_node.attribute("pano_yaw_deg").as_float();
   panorama.tiltYaw = proj_node.attribute("tilt_yaw_deg").as_float();
   panorama.tiltPitch = proj_node.attribute("tilt_pitch_deg").as_float();
   convertYaw(panorama.panoYaw);
   convertYaw(panorama.tiltYaw);
   ...
   *(datestr + length) = ' ';
   ...
   RawEdge edge;
   while (edge_node) {
      edge.angle = edge_node.attribute("yaw_deg").as_float();
      convertYaw(edge.angle);
      pid = _strings.addString(edge_node.attribute("pano_id").value());
      edge.panoid = _strings.getString(pid);
      panorama.edges.add(edge);
...
//------------------------------------------------------------------------------
PhotoMetadata* ImageDownloader::downloadPano (Panorama const& panorama, unsigned zoom) {
...
//------------------------------------------------------------------------------
PhotoMetadata* ImageDownloader::savePano (Panorama const& panorama) {
   // add it to the database
   PhotoMetadata* metadata = _database->addNewMetaData(panorama.location,
                                                       panorama.panoid,
                                                       panorama.origLocation,
                                                       panorama.captureDate,
                                                       panorama.panoYaw,
                                                       panorama.tiltYaw,
                                                       panorama.tiltPitch);
   // add its edges
   LinkedList<RawEdge>::ConstIterator eitr;
   for (eitr = panorama.edges.constIterator(); eitr.valid(); ++eitr) {
      _database->addEdge(metadata, eitr.cref().angle, eitr.cref().panoid);
   }
...
//------------------------------------------------------------------------------
PhotoMetadata* ImageDownloader::savePano (char const* panoid, unsigned zoom) {
   Panorama pano;
   ...
   for (unsigned i=1; i<_locationBuffer.filled(); ++i) {
      ...
      for (unsigned j=0; j<steps; ++j) {
         ...
         if (getPanoNear(loc, pano) and strcmp(lastpano, pano.panoid) != 0) {
            if ( !_database->findByPanoId(pano.location, pano.panoid) ) {
	       if (!metadataOnly) {
		  downloadPano(pano, 3);
	       } else {
		  savePano(pano);
...
Location ImageDownloader::calculateStep (Location const& loc1, Location const& loc2, float maxStep, unsigned& steps) {
   ...
   steps = 0;
   float abslat = abs(step.lat());
   float abslon = abs(step.lon());
   if (abslat <= abslon) {
